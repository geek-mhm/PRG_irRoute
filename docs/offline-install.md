...
Use an offline bundle when the target server cannot reach GitHub or its operating-system package repositories. The bundle contains a static Linux binary, the systemd unit, the loose reverse-path-filtering configuration, the Iran IPv4 seed, and an installer that does not use the network.
...
- installs and applies loose reverse-path filtering for dual-interface routing;
