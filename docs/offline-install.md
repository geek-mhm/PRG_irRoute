...
```sh
curl -LO https://github.com/geek-mhm/PRG_irRoute/releases/download/v0.1.0/irroute-0.1.0-linux-amd64.tar.gz
curl -LO https://github.com/geek-mhm/PRG_irRoute/releases/download/v0.1.0/SHA256SUMS
```
...
```sh
make offline-bundles VERSION=0.1.0
```

This creates:

- `dist/irroute-0.1.0-linux-amd64.tar.gz` for `x86_64` servers;
- `dist/irroute-0.1.0-linux-arm64.tar.gz` for `aarch64` servers;
...
```sh
scp dist/irroute-0.1.0-linux-amd64.tar.gz root@SERVER_IP:/root/
...
tar -xzf irroute-0.1.0-linux-amd64.tar.gz
cd irroute-0.1.0-linux-amd64
./install.sh
```
